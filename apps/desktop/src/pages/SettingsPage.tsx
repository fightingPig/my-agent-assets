import { Activity, FileText, FolderCog, Palette, RefreshCw, ScanSearch, ShieldCheck, TerminalSquare } from "lucide-react";
import { useEffect, useState } from "react";
import { diagnosticExportApply, diagnosticExportPreview, gitRemoteApply, gitRemotePreview, settingsApply, settingsLoad, settingsPreview } from "../app/data-api";
import type { ApplyResult, DesktopSettings, DiagnosticExportPreview, GitRemotePreview, SettingsPreview } from "../app/contracts";
...
export function SettingsPage({ demoMode = false }: { demoMode?: boolean }) {
  ...
  const handleDiagnosticPreview = async () => {
    ...
    try {
      ...
      setDiagnosticMessage("诊断包预览失败，请检查本地日志目录权限。");
    } finally {
  ...
  const handleRemotePreview = async () => {
    if (!settings || !remoteUrl.trim()) return;
    setRemoteBusy(true);
    setRemoteMessage("");
    try {
  ...
  const handleRemoteApply = async () => {
    if (!remotePreview?.canApply || !settings) return;
    setRemoteBusy(true);
    try {
  ...
  return (
    <div className="settings-workspace">
      ...
      <section className="panel settings-section"><div className="settings-section-title"><RefreshCw size={17} /><div><h3>同步设置</h3><p>本地 Git 仓库同步偏好</p></div></div><div className="settings-controls two"><label><span>默认分支</span><input data-no-drag="true" onChange={(event) => updateSetting("gitDefaultBranch", event.target.value)} style={noDragControl} value={settings.gitDefaultBranch} /></label><label><span>远程名称</span><input data-no-drag="true" onChange={(event) => updateSetting("gitRemote", event.target.value)} style={noDragControl} value={settings.gitRemote} /></label><label className="settings-control-wide"><span>远程仓库 URL</span><div className="path-picker-control"><input data-no-drag="true" onChange={(event) => { setRemoteUrl(event.target.value); setRemotePreview(null); setRemoteMessage(""); }} placeholder="git@github.com:owner/private-assets.git" style={noDragControl} value={remoteUrl} /><button className="asset-secondary-action" data-no-drag="true" disabled={remoteBusy || !remoteUrl.trim()} onClick={() => void handleRemotePreview()} style={noDragControl} type="button">预览配置</button></div><small>仅在确认后修改资产中心的 Git remote；不会执行 fetch、pull 或 push。</small></label></div>{remotePreview ? <ApplyConfirmationPanel actionLabel="确认配置远程仓库" canApply={remotePreview.canApply} description={`${remotePreview.previousUrl ?? "尚未配置"} → ${remotePreview.remoteUrl}`} isApplying={remoteBusy} onApply={() => void handleRemoteApply()} operationError={remotePreview.canApply ? null : remotePreview.warnings.join("；")} result={null} title="配置 Git remote" /> : null}{remoteMessage ? <p className={remoteMessage.includes("失败") ? "warning-text" : "success-text"} role="status">{remoteMessage}</p> : null}<div className="settings-toggle-list"><label><input checked={settings.allowPublicRemotePush} data-no-drag="true" onChange={(event) => updateSetting("allowPublicRemotePush", event.target.checked)} style={noDragControl} type="checkbox" /><span><strong>允许推送到公开远程仓库</strong><small>默认只允许已验证的 GitHub 私有仓库。开启后可推送到任意 Git remote，执行时会高亮公开或未知可见性风险。</small></span></label></div></section>
      ...
      <section className="panel settings-section"><div className="settings-section-title"><FileText size={17} /><div><h3>日志设置</h3><p>本地诊断日志、保留周期与脱敏导出</p></div></div><div className="settings-controls two"><label><span>日志级别</span><select data-no-drag="true" onChange={(event) => updateSetting("logLevel", event.target.value as DesktopSettings["logLevel"])} style={noDragControl} value={settings.logLevel}><option value="error">Error</option><option value="warn">Warn</option><option value="info">Info</option><option value="debug">Debug</option></select></label><label><span>保留周期</span><input data-no-drag="true" min={1} max={365} onChange={(event) => updateSetting("logRetentionDays", Number(event.target.value))} style={noDragControl} type="number" value={settings.logRetentionDays} /></label></div><div className="diagnostic-export-settings"><Activity size={17} /><div><strong>导出脱敏诊断包</strong><span>只包含版本、平台、状态摘要和脱敏审计日志。</span></div><button className="asset-secondary-action" data-no-drag="true" disabled={diagnosticBusy} onClick={() => void handleDiagnosticPreview()} style={noDragControl} type="button">预览导出</button></div>{diagnosticExport ? <div className="initialization-preview"><p>{diagnosticExport.includedFiles.map((file) => file.logicalPath).join("、") || "状态摘要"}</p><button className="asset-business-action" data-no-drag="true" disabled={diagnosticBusy || !diagnosticExport.canApply} onClick={() => void handleDiagnosticApply()} style={noDragControl} type="button">确认导出</button></div> : null}{diagnosticMessage ? <p className="settings-path-guidance" role="status">{diagnosticMessage}</p> : null}</section>
      <section className="panel settings-section"><div className="settings-section-title"><TerminalSquare size={17} /><div><h3>CLI 设置</h3><p>maa 命令行工具信息</p></div></div><div className="settings-controls two"><label><span>可执行文件</span><input data-no-drag="true" onChange={(event) => updateSetting("cliPath", event.target.value)} style={noDragControl} value={settings.cliPath} /></label><label><span>版本</span><input data-no-drag="true" readOnly style={noDragControl} value="0.1.1-beta.1" /></label></div>{saveMessage && stateLabel !== "保存失败" ? <p className={stateLabel === "预览失败" ? "warning-text" : "success-text"} role="status">{saveMessage}</p> : null}<div className="settings-actions"><button className="asset-business-action" data-no-drag="true" disabled={isPlanning || isApplying} onClick={handlePreviewSave} style={noDragControl} type="button">{isPlanning ? "生成中" : "生成保存预览"}</button></div>{preview || applyResult ? <ApplyConfirmationPanel actionLabel="确认保存设置" canApply={Boolean(preview?.canApply)} description={preview?.warnings[0] ?? preview?.plannedEffects.join("；") ?? "保存前必须生成并确认有效预览。"} isApplying={isApplying} onApply={handleApplySave} operationError={stateLabel === "保存失败" ? saveMessage : null} result={applyResult} title="保存本地设置" /> : null}</section>
    </div>
  );
}
