...
- `StaticActionButton` locations mark destructive apply command entry points without currently exposing executable behavior; Settings save, Mount Manager plan generation, and Backup Restore plan generation are handled by separate controlled actions.
...
- Destructive apply-style business actions remain intentionally disabled; Settings save, Mount Manager plan generation, and Backup Restore plan generation were added as controlled actions after the static freeze.
