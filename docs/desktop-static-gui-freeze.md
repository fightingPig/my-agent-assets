...
- `StaticActionButton` locations mark destructive apply command entry points without currently exposing executable behavior; Settings save, Scan Import plan generation, Mount Manager plan generation, and Backup Restore plan generation are handled by separate controlled actions.
